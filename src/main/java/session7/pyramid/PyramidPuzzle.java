...
/**
 * To solve a reduced version of the Pyramid Puzzle Instructions at
...
public class PyramidPuzzle extends BranchAndBound {
	 ...
	 * 
	 * @param board
	 *            Representation of the board for playing the puzzle
	 */
...
class PyramidBoard extends Node {
	...
	/**
	 * Inserts the values of a line from the pyramid It is call once per every row
	 * of the pyramid to initialize all the values
	 * 
	 * @param values
	 *            Values of a row of the pyramid
	 * @param row
	...
	private boolean prune() {
		for (int i = 0; i < n - 1; i++) // last row has notihng below
			for (int j = 0; j <= i; j++) {
				if (board[i][j] > 0 && board[i + 1][j] > 0 && board[i + 1][j + 1] > 0) {
					...
					if (board[i][j] == board[i + 1][j] + board[i + 1][j + 1])
						...
						valid = true;
					if (!valid)
						return true; // not valid -> must not continue
				}
			}
		return false;
	}
	...
	public ArrayList<Node> expand() {
		...
		while (nextRow >= 0 && nextCol >= 0 && board[nextRow][nextCol] > 0) {
			if (column - 1 < 0) {
				nextRow--;
				nextCol = nextRow;
			} else {
		...
		for (int k = 1; k < 10; k++) {
			testBoard = copyBoard(nextRow, nextCol, k);
			temp = new PyramidBoard(testBoard, depth+1, this.getID(), nextRow, nextCol);
			result.add(temp);
		}
