...
public class BoggleAllBacktrackingRepetitions extends Boggle {
	...
	public BoggleAllBacktrackingRepetitions(String dictionaryFileName, int concatenated, int repeatCell,
			int tableSize) {
		super(dictionaryFileName, tableSize);
		this.concatenated = concatenated;
		this.repeatCell = repeatCell;
		this.repeatedVisits = new int[tableSize][tableSize];
	...
	private void calculatePoints(String string, int k) {
		...
		if (k == 1) { // normal word
			...
			// System.out.println(string + "\t\t\t\t " + (wordClean.length() * k +
			// Math.pow(wordClean.length(), k)));
		}
